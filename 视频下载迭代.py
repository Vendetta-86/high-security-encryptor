...
class AdvancedDownloader:
    ...
    # UI 更新辅助
    # =========================
    def _set_status(self, text: str, fg: str = "blue"):
        self.root.after(0, lambda t=text, c=fg: self.status_label.config(text=t, fg=c))

    def _set_progress(self, value: float):
        v = max(0.0, min(100.0, float(value)))
        self.root.after(0, lambda vv=v: self.progress_bar.configure(value=vv))
    ...
    def _ffprobe_duration(self, file_path: str) -> Optional[float]:
        with self.lock:
            ffprobe = self.env_info.get("ffprobe") or "ffprobe"
        try:
            cp = subprocess.run(
    ...
    def _parse_ffmpeg_out_time_to_sec(self, kv: Dict[str, str]) -> Optional[float]:
        try:
            if "out_time_us" in kv:
                return max(0.0, float(kv["out_time_us"]) / 1_000_000.0)
            if "out_time_ms" in kv:
                # ffmpeg 这里字段名历史原因常见为 ms，但实际数值通常是微秒
                raw = float(kv["out_time_ms"])
                if raw > 100000:
                    return max(0.0, raw / 1_000_000.0)
    ...
    def _run_ffmpeg(
        ...
        self._set_status(f"{progress_title}...", "blue")
        ...
        try:
            ...
            if proc.stderr is None:
                raise RuntimeError("ffmpeg stderr pipe not available")
            ...
            while True:
                ...
                if time.time() - start_ts > timeout and proc.poll() is None:
                    try:
                        proc.kill()
                    except Exception:
                        pass
                ...
                if "=" in s:
                    ...
                    now = time.time()
                    cur_sec = self._parse_ffmpeg_out_time_to_sec(kv)
                    speed = kv.get("speed", "").strip()

                    if key == "progress" and value.strip() == "end":
                        self._set_progress(100.0)
                        done_text = "✅ ffmpeg 执行完成"
                        if source_duration and cur_sec is not None:
                            done_text += f" | {self._format_seconds(cur_sec)}/{self._format_seconds(source_duration)}"
                        if speed:
                            done_text += f" | speed={speed}"
                        self._safe_log(done_text)
                        continue

                    if cur_sec is not None and (now - last_ui_ts >= 0.25):
                        if source_duration and source_duration > 0:
                            percent = min(99.0, max(0.0, cur_sec / source_duration * 100.0))
                            self._set_progress(percent)
                            status = (
                                f"{progress_title}: {percent:.1f}% | "
                                f"{self._format_seconds(cur_sec)}/{self._format_seconds(source_duration)}"
                            )
                        else:
                            status = f"{progress_title}: {self._format_seconds(cur_sec)}"
                        if speed:
                            status += f" | speed={speed}"
                        self._set_status(status, "blue")
                        last_ui_ts = now

                    if cur_sec is not None and (now - last_log_ts >= 2.0):
                        if source_duration and source_duration > 0:
                            percent = min(99.0, max(0.0, cur_sec / source_duration * 100.0))
                            msg = (
                                f"🔄 {progress_title}: {percent:.1f}% | "
                            ...
                            msg = f"🔄 {progress_title}: {self._format_seconds(cur_sec)}"
                        if speed:
                            msg += f" | speed={speed}"
                        self._safe_log(msg)
                        last_log_ts = now
            ...
            if return_code != 0:
                ...
                tail = "\n".join(last_lines[-12:]).strip()
                err = tail if tail else "unknown error"
                self._safe_log("❌ ffmpeg 失败：" + err[-1200:])
                self._set_status("ffmpeg 处理失败", "red")
                return False
            ...
            if self._file_ready(expected_output) or expected_output is None:
                if expected_output:
                    self.last_success_media = expected_output
                self._set_progress(100.0)
                self._set_status("ffmpeg 处理完成", "green")
                return True

            self._safe_log("⚠️ ffmpeg 已结束，但目标文件未稳定落盘")
            self._set_status("ffmpeg 已结束，但目标文件未稳定落盘", "orange")
            return False
            ...
            self._set_status("ffmpeg 超时", "red")
            return False
            ...
            self._set_status("ffmpeg 异常", "red")
            return False
    ...
    def _attach_cover_if_possible(self, mp4_path: str, cover_jpg: str) -> bool:
        ...
        dur = self._ffprobe_duration(mp4_path)
        ok = self._run_ffmpeg([
            "-y",
            "-i", mp4_path,
            "-i", cover_jpg,
            ...
            "-movflags", "+faststart",
            tmp
        ], expected_output=tmp, timeout=600, progress_title="写入封面", source_duration=dur)
        if not ok:
    ...
    def stop_task(self):
        ...
        if self._file_ready(target):
            ...
            self._set_status("文件已生成，正在完成收尾...", "green")
            return

        self.stop_requested = True
        self._set_status("正在停止...", "red")
    ...
    def _download_compat_accurate(self, ydl: yt_dlp.YoutubeDL, url: str, cap: Optional[int]):
        ...
        src_duration = self._ffprobe_duration(in_path)
        self.write_log(
            f"🔬 实际落盘 codec：v={vcodec}  a={acodec}"
            + (f" | 时长={self._format_seconds(src_duration)}" if src_duration else "")
        )
        ...
        if not need_v and not need_a:
            ...
            ok = self._run_ffmpeg([
                ...
                "-movflags", "+faststart",
                out_mp4
            ], expected_output=out_mp4, progress_title="兼容封装", source_duration=src_duration)
            if not ok:
    ...
    def p_hook(self, d):
        ...
        if status == "downloading":
            ...
            try:
                ...
                self._set_status(f"下载进度: {p:.1f}%", "blue")
            except Exception:
                pass
        elif status == "finished":
            self._set_status("下载完成，正在收尾...", "blue")
    ...
    def pp_hook(self, d):
        ...
        if status in ("started", "processing"):
            self._set_status(f"后处理中: {pp}", "blue")
        elif status == "finished":
            self._set_status(f"后处理完成: {pp}", "blue")
