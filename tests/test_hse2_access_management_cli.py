...
class HSE2AccessManagementCliTests(unittest.TestCase):
    ...
    def test_access_cli_requires_exact_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            fixture = _create_fixture(Path(temp_dir))
            stdout = io.StringIO()
            stderr = io.StringIO()
            ...
            with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                exit_code = access_main([
                    ...
                    str(fixture.container),
                    "--output",
                    str(fixture.disabled_container),
                    "--confirm",
                    "wrong phrase",
                ])
            ...
            self.assertFalse(fixture.disabled_container.exists())

    def test_access_cli_removes_wrappers_and_marks_container(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            fixture = _create_fixture(Path(temp_dir))

            payload = _disable_access(fixture.container, fixture.disabled_container)
            ...
            listed = _list_wrappers(fixture.disabled_container)
            self.assertTrue(listed["access_destroyed"])
            self.assertEqual(listed["wrapper_count"], 0)
            self.assertEqual(_open_container(fixture.disabled_container, fixture.keyfile, fixture.restored_dir), 2)
...
class _Fixture:
    def __init__(self, base: Path) -> None:
        ...
        self.disabled_container = base / "disabled.hse2"
...
def _disable_access(container: Path, output: Path) -> dict[str, object]:
    stdout = io.StringIO()
    with contextlib.redirect_stdout(stdout):
        exit_code = access_main([
            "destroy",
    ...
    if exit_code != 0:
        raise AssertionError(f"access command failed: {exit_code}")
    return json.loads(stdout.getvalue())
