...
MAX_AGG_PAGES = 10                 # 聚合最多页数
MAX_FULL_FETCH_PAGES = 50          # “定位搜索”拉全量页数上限（防止无限/过大）
# ==========================
...
class App:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("专业下载器（完整整合版：全量定位+自动跳页+下载/歌词/ID3/缓存/聚合）")
        self.root.geometry("1460x1060")

        # 状态
        self.keyword = ""
        self.current_page = 1

        self.downloaded_ids = set()             # download_cache.txt
        self.selected_by_id: dict[str, bool] = {}  # state.json
        self.song_cache: dict[str, dict] = {}      # state.json（id->简化元数据）
        ...
        # 聚合
        self.agg_pages = 5
        self.agg_mode = "range"           # range/custom
        self.agg_custom_pages = "1-5"
        self.aggregate_cancel_event = threading.Event()

        # 当前显示的结果集合（可能是某页结果，也可能是聚合集合）
        self.last_result_set: list[dict] = []
        self.unaggregated_snapshot_items: list[dict] | None = None
        self.is_aggregated_view = False

        # 全量定位缓存：key = f"{keyword}||{limit}"
        self.full_results_cache: dict[str, list[dict]] = {}
        self.full_id_to_page: dict[str, dict[str, int]] = {}  # key -> {song_id: page}
        self.full_fetch_lock = threading.Lock()

        # 定位匹配结果与索引
        self.locate_match_ids: list[str] = []
        self.locate_index = -1
        self.locate_status_var = tk.StringVar(value="命中：0 | 当前：0/0")

        # 自动跳页滚动
        self.pending_scroll_to_id: str | None = None
    ...
    def _build_ui(self):
        ...
        ttk.Entry(api_row, width=38, textvariable=self.base_url_var).pack(side="left", padx=(0, 10))

        ttk.Label(api_row, text="健康检测 Path：").pack(side="left")
        self.health_path_var = tk.StringVar(value=self.health_path)
        ttk.Entry(api_row, width=10, textvariable=self.health_path_var).pack(side="left", padx=(0, 10))
        ...
        ttk.Entry(dl_row, width=86, textvariable=self.download_dir_var).pack(side="left", padx=(0, 10), fill="x", expand=True)
        ...
        ttk.Label(search_row, text="limit：").pack(side="left", padx=(14, 0))
        self.limit_var = tk.StringVar(value=str(self.search_limit))
        ttk.Entry(search_row, width=6, textvariable=self.limit_var).pack(side="left", padx=(6, 0))
        ttk.Label(search_row, text="（调大=一次拿更多）", foreground="gray").pack(side="left", padx=10)

        if not MUTAGEN_OK:
            ttk.Label(search_row, text="（未安装 mutagen：将跳过 ID3）", foreground="gray").pack(side="right")

        # 聚合行
        agg_row = ttk.Frame(top)
        agg_row.pack(fill="x", pady=(6, 0))

        ttk.Label(agg_row, text="聚合：").pack(side="left")

        self.agg_mode_var = tk.StringVar(value=self.agg_mode)
        ttk.Radiobutton(agg_row, text="连续页", variable=self.agg_mode_var, value="range",
                        command=self._on_agg_mode_change).pack(side="left", padx=(6, 0))
        ttk.Radiobutton(agg_row, text="自定义页码", variable=self.agg_mode_var, value="custom",
                        command=self._on_agg_mode_change).pack(side="left", padx=(6, 0))

        ttk.Label(agg_row, text="  连续页数N（<=10）：").pack(side="left", padx=(14, 0))
        self.agg_n_var = tk.StringVar(value=str(self.agg_pages))
        self.agg_n_entry = ttk.Entry(agg_row, width=6, textvariable=self.agg_n_var)
        self.agg_n_entry.pack(side="left", padx=(6, 10))

        ttk.Label(agg_row, text="自定义页码（<=10页，如 1,3-5,8）：").pack(side="left")
        self.agg_custom_var = tk.StringVar(value=self.agg_custom_pages)
        self.agg_custom_entry = ttk.Entry(agg_row, width=22, textvariable=self.agg_custom_var)
        self.agg_custom_entry.pack(side="left", padx=(6, 10))

        self.btn_aggregate = ttk.Button(agg_row, text="聚合并显示", command=self.aggregate_current_query_async)
        self.btn_aggregate.pack(side="left", padx=6)
        self.btn_cancel_aggregate = ttk.Button(agg_row, text="中止聚合并还原", command=self.cancel_aggregate_and_restore)
        self.btn_cancel_aggregate.pack(side="left", padx=6)
        ttk.Button(agg_row, text="恢复为当前页结果", command=self.restore_current_page_result).pack(side="left", padx=6)

        # 定位行（全量）
        locate_row = ttk.Frame(top)
        locate_row.pack(fill="x", pady=(6, 0))

        ttk.Label(locate_row, text="结果内搜索（全量定位，不隐藏）：").pack(side="left")
        self.locate_query_var = tk.StringVar(value="")
        self.locate_entry = ttk.Entry(locate_row, width=40, textvariable=self.locate_query_var)
        self.locate_entry.pack(side="left", padx=(6, 8))
        self.locate_entry.bind("<Return>", lambda e: self.locate_in_results_all_async())

        ttk.Button(locate_row, text="定位/选中匹配", command=self.locate_in_results_all_async).pack(side="left")
        ttk.Button(locate_row, text="上一个结果", command=lambda: self.goto_match(-1)).pack(side="left", padx=6)
        ttk.Button(locate_row, text="下一个结果", command=lambda: self.goto_match(1)).pack(side="left", padx=6)

        ttk.Label(locate_row, textvariable=self.locate_status_var, foreground="blue").pack(side="left", padx=14)
        ttk.Label(locate_row, text="（上/下一个会自动跳页并定位展示）", foreground="gray").pack(side="left", padx=10)
        ...
        ttk.Button(action, text="全选（当前页/当前集合）", command=self.select_all_current_view).pack(side="left")
        ttk.Button(action, text="取消全选（当前页/当前集合）", command=self.unselect_all_current_view).pack(side="left", padx=6)
        ...
        self.btn_download = ttk.Button(action, text="下载选中（所有页/跨页）", command=self.download_selected_all_pages)
        ...
        self._on_agg_mode_change()

    def _on_agg_mode_change(self):
        mode = (self.agg_mode_var.get() or "range").strip()
        if mode == "range":
            self.agg_n_entry.config(state="normal")
            self.agg_custom_entry.config(state="disabled")
        else:
            self.agg_n_entry.config(state="disabled")
            self.agg_custom_entry.config(state="normal")
    ...
    def ui_locate_status(self, hit: int, idx: int, total: int):
        self.ui_q.put(("locate_status", hit, idx, total))
        ...
        try:
            while True:
                ...
                if typ == "log":
                    ...
                    # 渲染后如有 pending scroll，则滚动到目标
                    if self.pending_scroll_to_id:
                        sid = self.pending_scroll_to_id
                        self.pending_scroll_to_id = None
                        self.root.after(60, lambda _sid=sid: self.scroll_to_song(_sid))

                elif typ == "locate_status":
                    hit, idx, total = item[1], item[2], item[3]
                    self.locate_status_var.set(f"命中：{hit} | 当前：{idx}/{total}")
    ...
    def _load_state(self):
        ...
        try:
            ...
            self.agg_mode = data.get("agg_mode", "range") or "range"
            self.agg_pages = int(data.get("agg_pages", 5))
            self.agg_custom_pages = data.get("agg_custom_pages", "1-5") or "1-5"
            ...
            self.agg_mode_var.set(self.agg_mode)
            self.agg_n_var.set(str(self.agg_pages))
            self.agg_custom_var.set(self.agg_custom_pages)
            self._on_agg_mode_change()
    ...
    def _save_state(self):
        try:
            data = {
                ...
                "agg_mode": (self.agg_mode_var.get() or "range").strip(),
                "agg_pages": self.agg_pages,
                "agg_custom_pages": (self.agg_custom_var.get() or "1-5").strip(),
            }
            tmp = STATE_FILE + ".tmp"
            with open(tmp, "w", encoding="utf-8") as f:
    ...
    def _refresh_config_from_ui(self):
        ...
        # agg N
        sn = (self.agg_n_var.get() or "").strip()
        try:
            n = int(sn)
            if n <= 0:
                n = 1
    ...
    def _ensure_api_ready_worker(self):
        ...
        if shutil.which("node") is None:
            self.ui_log("API 不可达：系统没有 node，无法自动启动。\n")
            return

        proj = find_api_project_dir(self.api_dir_hint or None)
        if not proj:
            self.ui_log("API 不可达：未找到可启动项目目录（需 app.js 与 package.json）。\n")
            self.ui_log("可点“选择 API 目录（可选）”指定一次。\n")
            return
    ...
    # --------------- 选择保存 ---------------
    def _persist_current_view_selection(self):
    ...
    def select_all_current_view(self):
        for row in self.ui_rows.values():
            row["var"].set(True)
        self._persist_current_view_selection()

    def unselect_all_current_view(self):
        for row in self.ui_rows.values():
            row["var"].set(False)
        self._persist_current_view_selection()

    # --------------- 搜索/分页 ---------------
    def search(self, page: int, preserve_locate: bool = False, pending_scroll_id: str | None = None):
        """
        preserve_locate=True：用于“上/下一个结果自动跳页”，不会清空 locate 命中列表/索引。
        pending_scroll_id：搜索渲染后自动滚动到该 song_id。
        """
        self._refresh_config_from_ui()
        ...
        if not ok:
            ...
            self.ui_message("提示", "正在确保 API 可用（可能会自动启动）。稍后再点一次。", "info")
            return
        ...
        # 关键词变化则清理全量缓存（避免旧映射）
        if kw != self.keyword:
            old_key = f"{self.keyword}||{self.search_limit}"
            self.full_results_cache.pop(old_key, None)
            self.full_id_to_page.pop(old_key, None)
        ...
        self.pending_scroll_to_id = pending_scroll_id

        threading.Thread(target=self._search_worker, args=(kw, page, preserve_locate), daemon=True).start()

    def _search_worker(self, kw: str, page: int, preserve_locate: bool):
        with self.search_lock:
            ...
            try:
                ...
                self._set_result_set_and_render(songs, preserve_locate=preserve_locate)
    ...
    # --------------- 结果集合渲染 ---------------
    def _set_result_set_and_render(self, items: list[dict], preserve_locate: bool = False):
        self.last_result_set = list(items)
        if not preserve_locate:
            self.locate_match_ids = []
            self.locate_index = -1
            self.ui_locate_status(0, 0, 0)
        self.ui_q.put(("render_list", self.last_result_set))
    ...
    def _parse_custom_pages(self, s: str) -> list[int]:
    ...
    def aggregate_current_query_async(self):
        ...
        kw = self.entry.get().strip()
        if not kw:
            self.ui_message("提示", "请先输入关键词（可先搜索再聚合，也可直接聚合）", "warning")
            return

        ok, _ = self.check_api_reachable()
        if not ok:
            self.ensure_api_ready_async()
            self.ui_message("提示", "API 不可达，正在尝试自动启动。等可达后再点聚合。", "warning")
            return
        ...
        if mode == "range":
            n = self.agg_pages
            pages = list(range(1, min(MAX_AGG_PAGES, n) + 1))
        else:
            pages = self._parse_custom_pages((self.agg_custom_var.get() or "").strip())
            if not pages:
                self.ui_message("提示", "自定义页码为空或无效。例如：1,3-5,8", "warning")
                return

        # 记录聚合前快照
        if self.unaggregated_snapshot_items is None:
            self.unaggregated_snapshot_items = list(self.last_result_set) if self.last_result_set else []

        self.is_aggregated_view = False
        self.aggregate_cancel_event.clear()
        threading.Thread(target=self._aggregate_worker, args=(kw, pages), daemon=True).start()

    def cancel_aggregate_and_restore(self):
        self.aggregate_cancel_event.set()
        self.ui_log("已请求中止聚合：将还原未聚合视图…\n")

        snap = self.unaggregated_snapshot_items
        if snap is not None:
            self.is_aggregated_view = False
            self._set_result_set_and_render(snap, preserve_locate=False)
            self.ui_log("已还原为未聚合结果集合。\n")
        else:
            self.restore_current_page_result()

    def _aggregate_worker(self, kw: str, pages: list[int]):
        self.ui_set_busy(True)
        try:
            limit = self.search_limit
            self.ui_log(f"开始聚合：关键词='{kw}' | pages={pages} | limit={limit}\n")
            merged: list[dict] = []
            seen = set()
            ...
            for idx, p in enumerate(pages, start=1):
                if self.aggregate_cancel_event.is_set():
                    ...
                    if self.unaggregated_snapshot_items is not None:
                        self.is_aggregated_view = False
                        self._set_result_set_and_render(self.unaggregated_snapshot_items, preserve_locate=False)
                    return

                offset = (p - 1) * limit
                data = with_retry(lambda: self.api_get_json("/search", {"keywords": kw, "limit": limit, "offset": offset}), tries=3)
                songs = data.get("result", {}).get("songs", []) or []
                self.ui_log(f"聚合：第 {idx}/{len(pages)} 个页码（page={p}）返回 {len(songs)} 条\n")
            ...
            self.ui_log(f"聚合完成：共 {len(merged)} 条（pages={pages}）\n")
            self._set_result_set_and_render(merged, preserve_locate=False)
    ...
    # --------------- 全量定位（搜全部结果） + 映射 ---------------
    def _item_to_search_blob(self, song: dict) -> str:
        sid, name, artists, album, fee = normalize_song_fields(song)
        return f"{name} {artists} {album}".lower()

    def locate_in_results_all_async(self):
        q = (self.locate_query_var.get() or "").strip().lower()
        if not q:
            self.ui_message("提示", "请输入定位关键词", "warning")
            return

        kw = (self.entry.get() or "").strip() or (self.keyword or "").strip()
        if not kw:
            self.ui_message("提示", "请先输入主关键词并搜索（或至少输入关键词）", "warning")
            return

        ok, _ = self.check_api_reachable()
        if not ok:
            self.ensure_api_ready_async()
            self.ui_message("提示", "API 不可达，正在尝试自动启动。稍后再定位。", "warning")
            return

        self._refresh_config_from_ui()
        threading.Thread(target=self._locate_all_worker, args=(kw, q), daemon=True).start()

    def _fetch_full_results(self, keyword: str, limit: int) -> list[dict]:
        """
        拉取该关键词的“全部结果”：从 page=1 开始一直拉到空结果或达到 MAX_FULL_FETCH_PAGES。
        并建立 song_id -> page 映射，缓存到内存。
        """
        cache_key = f"{keyword}||{limit}"
        cached = self.full_results_cache.get(cache_key)
        if cached is not None:
            return cached

        with self.full_fetch_lock:
            cached2 = self.full_results_cache.get(cache_key)
            ...
            for page in range(1, MAX_FULL_FETCH_PAGES + 1):
                offset = (page - 1) * limit
                data = with_retry(lambda: self.api_get_json("/search", {"keywords": keyword, "limit": limit, "offset": offset}), tries=3)
                songs = data.get("result", {}).get("songs", []) or []
                self.ui_log(f"全量定位：拉取第 {page} 页，返回 {len(songs)} 条\n")
                if not songs:
                    break

                for s in songs:
                    sid = str(s.get("id") or "")
                    k = sid if sid else self._dedup_key(s)
                    if k in seen:
                        continue
                    seen.add(k)
                    ...
                    if sid:
                        id_to_page[sid] = page
                        self.song_cache[sid] = compact_song_for_state(s)

            self.full_results_cache[cache_key] = all_songs
            self.full_id_to_page[cache_key] = id_to_page
            self._save_state()
            return all_songs

    def _locate_all_worker(self, keyword: str, locate_q: str):
        self.ui_set_busy(True)
        try:
            limit = self.search_limit
            cache_key = f"{keyword}||{limit}"
            songs = self._fetch_full_results(keyword, limit)
            id_to_page = self.full_id_to_page.get(cache_key, {})
            ...
            for s in songs:
                ...
                if not sid:
                    continue
                if locate_q in self._item_to_search_blob(s):
            ...
            self.ui_message("定位结果", f"搜索到 {len(match_ids)} 个结果", "info")
            self.ui_log(f"定位（全量）：关键词='{keyword}' | 命中 {len(match_ids)} | 定位词='{locate_q}'\n")

            # 自动勾选（不取消其它）
            for sid in match_ids:
                self.selected_by_id[sid] = True
                row = self.ui_rows.get(sid)
                if row:
                    row["var"].set(True)
            ...
            # 保存命中列表
            self.locate_match_ids = match_ids
            self.locate_index = 0 if match_ids else -1
            ...
            self.ui_locate_status(hit, 1, hit)

            # 自动跳到第一个命中所在页并滚动
            first_sid = match_ids[0]
            page = id_to_page.get(first_sid)
            if page:
                self.search(page, preserve_locate=True, pending_scroll_id=first_sid)
            else:
                self.ui_log("警告：无法定位到页码（id->page 缺失）。\n")

        except Exception as e:
            self.ui_message("错误", f"全量定位失败：{e}", "error")
        finally:
            self.ui_set_busy(False)

    # --------------- 上/下一个结果：自动跳页并展示 ---------------
    def goto_match(self, delta: int):
        if not self.locate_match_ids:
            self.ui_message("提示", "没有定位结果：请先点“定位/选中匹配”", "warning")
            return
        n = len(self.locate_match_ids)
        if n <= 0:
            return

        if self.locate_index < 0:
            self.locate_index = 0
        else:
            self.locate_index = (self.locate_index + delta) % n

        sid = self.locate_match_ids[self.locate_index]
        self.ui_locate_status(n, self.locate_index + 1, n)

        # 当前关键字 + limit 的映射
        kw = (self.keyword or self.entry.get().strip() or "").strip()
        cache_key = f"{kw}||{self.search_limit}"
        id_to_page = self.full_id_to_page.get(cache_key, {})

        # 如果当前列表已有该 sid，直接滚动
        if sid in self.ui_rows:
            self.scroll_to_song(sid)
            return

        # 否则自动跳页到它所在页
        page = id_to_page.get(sid)
        if not page:
            self.ui_log(f"无法获取命中项所在页：{sid}（建议重新点一次“定位/选中匹配”刷新全量映射）\n")
            return

        self.ui_log(f"自动跳页：命中项 {sid} 位于第 {page} 页\n")
        self.search(page, preserve_locate=True, pending_scroll_id=sid)

    def scroll_to_song(self, song_id: str):
        row = self.ui_rows.get(song_id)
        if not row:
            return
        frame = row.get("frame")
        if not frame:
    ...
    def _render_list(self, songs: list[dict]):
        ...
        for song in songs:
            ...
            row_frame = ttk.Frame(self.list_frame, padding=(6, 4))
            ...
            ttk.Checkbutton(row_frame, variable=var, command=on_toggle).pack(side="left")
            ttk.Label(row_frame, text=text).pack(side="left", padx=(6, 10), fill="x", expand=True)

            pb = ttk.Progressbar(row_frame, length=320, mode="determinate")
            pb.pack(side="left", padx=(0, 10))

            st = ttk.Label(row_frame, text="")
            st.pack(side="left")

            self.ui_rows[sid] = {"var": var, "progress": pb, "status": st, "frame": row_frame}
    ...
    def download_selected_all_pages(self):
        ...
        if not ok:
            self.ensure_api_ready_async()
            self.ui_message("提示", "正在确保 API 可用（可能会自动启动）。稍后再点下载。", "info")
            return
        ...
        if missing:
            self.ui_log(f"注意：有 {missing} 首歌缺少元数据缓存（建议先翻页搜到它或做一次全量定位）。\n")
        if not jobs:
    ...
    def _download_one_by_compact(self, c: dict):
        ...
        if (not self.force_redownload.get()) and (song_id in self.downloaded_ids):
            if audio_path_existing:
                ...
                self.ui_log(f"{safe_title} 缓存命中但音频不存在，已修复缓存并重下\n")
